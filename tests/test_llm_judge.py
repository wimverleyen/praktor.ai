...
class TestPHIGuard:
    def test_local_model_no_warning(self, caplog):
        ...
        with patch("praktor.governance.evaluators.llm_judge.AsyncLLMAdapter"):
    ...
    def test_cloud_model_emits_warning(self):
        ...
        with patch.dict(os.environ, env_clean, clear=True):
            with patch("praktor.governance.evaluators.llm_judge.AsyncLLMAdapter"):
                with patch("praktor.governance.evaluators.llm_judge.log") as mock_log:
    ...
    def test_air_gapped_local_model_ok(self):
        from praktor.governance.evaluators.llm_judge import SafetyJudge
...
class TestLLMJudgeEvaluator:
    ...
    async def test_score_pass(self):
        ...
        with patch("praktor.governance.evaluators.llm_judge.AsyncLLMAdapter") as Adapter:
    ...
    async def test_timeout_fail_open(self):
        ...
        with patch("praktor.governance.evaluators.llm_judge._JUDGE_TIMEOUT_S", 0.01):
            result = await judge.score(prompt="p", response="r")
    ...
    async def test_parse_error_fail_closed(self):
        from praktor.governance.evaluators.llm_judge import HelpfulnessJudge
...
class TestDomainJudges:
    ...
    async def test_faithfulness_threshold_6(self):
        ...
        with patch("praktor.governance.evaluators.llm_judge.AsyncLLMAdapter") as A:
    ...
    def test_judge_names(self):
        from praktor.governance.evaluators.llm_judge import (
            FaithfulnessJudge,
            HelpfulnessJudge,
            SafetyJudge,
        )
...
class TestEvaluatorsInit:
    ...
    def test_judge_result_dataclass(self):
        from praktor.governance.evaluators import JudgeResult
